...
export function ConfirmModal({ open, title, message, confirmLabel = '確認', cancelLabel = '取消', variant = 'danger', onConfirm, onCancel }: ConfirmModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center" onClick={onCancel}>
      ...
      <div
        className="relative w-full max-w-sm glass border border-nms-border rounded-xl shadow-2xl p-6 animate-fadeIn"
      ...
      >
        ...
        <h3 className="text-[14px] font-bold text-nms-text mb-1">{title}</h3>
        <p className="text-[11px] text-nms-text-muted mb-5 leading-relaxed">{message}</p>
        <div className="flex gap-2">
          <button
            onClick={onCancel}
            className="flex-1 px-4 py-2 rounded-lg bg-nms-card border border-nms-border text-[12px] text-nms-text-dim hover:bg-nms-hover transition-colors"
          >
            {cancelLabel}
          </button>
          <button
            onClick={onConfirm}
            className={`flex-1 px-4 py-2 rounded-lg ${style.btn} text-[12px] text-white font-medium transition-colors`}
          >
            {confirmLabel}
          </button>
        </div>
