import { Save, Clock, Play, Download, Upload, Trash2, Copy } from 'lucide-react';
    ...
    <div className="space-y-6 animate-fadeIn">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-nms-text-muted mt-1">管理所有遊戲存檔，支援備份、匯入匯出</p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => addNotification('匯入功能開發中...', 'info')} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-nms-card border border-nms-border text-nms-text-dim text-sm hover:bg-nms-hover transition-colors">
            <Upload size={14} /> 匯入存檔
          </button>
        </div>
      </div>
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button onClick={() => addNotification('刪除功能需二次確認', 'error')} className="p-2 rounded-lg text-nms-text-muted hover:text-nms-red hover:bg-nms-hover transition-colors" title="刪除">
                    <Trash2 size={14} />
                  </button>
                </div>
              </div>
